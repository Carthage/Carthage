...
public struct UpdateOptions: OptionsType {
	...
	public var buildOptions: BuildOptions {
		return BuildOptions(configuration: configuration, buildPlatform: buildPlatform, skipCurrent: true, dependencyName: dependencyName, colorOptions: checkoutOptions.colorOptions, verbose: verbose, directoryPath: checkoutOptions.directoryPath)
	}
	...
	public static func create(configuration: String)(buildPlatform: BuildPlatform)(dependencyName: String?)(verbose: Bool)(buildAfterUpdate: Bool)(checkoutOptions: CheckoutOptions) -> UpdateOptions {
		return self(buildAfterUpdate: buildAfterUpdate, configuration: configuration, buildPlatform: buildPlatform, dependencyName: dependencyName, verbose: verbose, checkoutOptions: checkoutOptions)
	}
	...
	public static func evaluate(m: CommandMode) -> Result<UpdateOptions, CommandantError<CarthageError>> {
		return create
			...
			<*> m <| Option(key: "dependency", defaultValue: nil, usage: "the dependency to build (ignored if --no-build option is present)")
