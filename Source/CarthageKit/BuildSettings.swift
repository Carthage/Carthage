...
public struct BuildSettings {
	...
	public static func load(with arguments: BuildArguments, for action: BuildArguments.Action? = nil) -> SignalProducer<BuildSettings, CarthageError> {
		...
		return task.launch()
			.ignoreTaskData()
			// If xcodebuild doesn't support a specific destination for the destination specifier,
			// it will return an exit code 70. Just eat that and continue.
			.flatMapError({ (error) -> SignalProducer<Data, TaskError> in
				switch error {
				case let .shellTaskFailed(_, exitCode, standardError):
					if exitCode == 70 {
						return SignalProducer(value: standardError?.data(using: .utf8) ?? Data())
					}
					fallthrough
				default:
