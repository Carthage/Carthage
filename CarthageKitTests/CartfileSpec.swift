...
class CartfileSpec: QuickSpec {
	override func spec() {
		...
		it("should parse a Cartfile") {
            ...
            expect(depReactiveCocoa.version).to(equal(VersionSpecifier.AtLeast(Version(major: 2, minor: 3, patch: 1, raw: "2.3.1"))))
            ...
            expect(depMantle.version).to(equal(VersionSpecifier.CompatibleWith(Version(major: 1, minor: 0, patch: 0, raw: "1.0.0"))))
            ...
            expect(depLibextobjc.version).to(equal(VersionSpecifier.Exactly(Version(major: 0, minor: 4, patch: 1, raw: "0.4.1"))))
...
class VersionSpec: QuickSpec {
	override func spec() {
		it("should order versions correctly") {
            let version = Version(major: 2, minor: 1, patch: 1, raw: "2.1.1")

            expect(version).to(beLessThan(Version(major: 3, minor: 0, patch: 0, raw: "3.0.0")))
            expect(version).to(beLessThan(Version(major: 2, minor: 2, patch: 0, raw: "3.2.0")))
            expect(version).to(beLessThan(Version(major: 2, minor: 1, patch: 2, raw: "2.1.2")))

			expect(version).to(beGreaterThan(Version(major: 1, minor: 2, patch: 2, raw: "1.2.2")))
			expect(version).to(beGreaterThan(Version(major: 2, minor: 0, patch: 2, raw: "2.0.2")))
            expect(version).to(beGreaterThan(Version(major: 2, minor: 1, patch: 0, raw: "2.1.0")))

            expect(version).to(beLessThan(Version(major: 10, minor: 0, patch: 0, raw: "10.0.0")))
            expect(version).to(beLessThan(Version(major: 2, minor: 10, patch: 1, raw: "2.10.1")))
            expect(version).to(beLessThan(Version(major: 2, minor: 1, patch: 10, raw: "2.1.10")))
		}
	}
}
...
class VersionSpecifierSpec: QuickSpec {
	override func spec() {
        let versionOne = Version(major: 1, minor: 3, patch: 2, raw: "1.3.2")
        let versionTwoZero = Version(major: 2, minor: 0, patch: 2, raw: "2.0.2")
        let versionTwoOne = Version(major: 2, minor: 1, patch: 1, raw: "2.1.1")
        let versionTwoTwo = Version(major: 2, minor: 2, patch: 0, raw: "2.2.0")
        let versionThree = Version(major: 3, minor: 0, patch: 0, raw: "3.0.0")
