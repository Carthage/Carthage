...
public struct Resolver {
	private let versionsForDependency: (ProjectIdentifier, Bool) -> SignalProducer<PinnedVersion, CarthageError>
	private let resolvedGitReference: (ProjectIdentifier, String, Bool) -> SignalProducer<PinnedVersion, CarthageError>
	private let cartfileForDependency: (Dependency<PinnedVersion>, Bool) -> SignalProducer<Cartfile, CarthageError>
	...
	///                         latest object.
	public init(versionsForDependency: (ProjectIdentifier, Bool) -> SignalProducer<PinnedVersion, CarthageError>, cartfileForDependency: (Dependency<PinnedVersion>, Bool) -> SignalProducer<Cartfile, CarthageError>, resolvedGitReference: (ProjectIdentifier, String, Bool) -> SignalProducer<PinnedVersion, CarthageError>) {
	...
	public func resolveDependenciesInCartfile(cartfile: Cartfile, verbose: Bool) -> SignalProducer<Dependency<PinnedVersion>, CarthageError> {
		return nodePermutationsForCartfile(cartfile, verbose: verbose)
			|> flatMap(.Concat) { rootNodes -> SignalProducer<Event<DependencyGraph, CarthageError>, CarthageError> in
				return self.graphPermutationsForEachNode(rootNodes, dependencyOf: nil, basedOnGraph: DependencyGraph(), verbose: verbose)
	...
	/// exist for each).
	private func nodePermutationsForCartfile(cartfile: Cartfile, verbose: Bool) -> SignalProducer<[DependencyNode], CarthageError> {
				...
				let allowedVersions = SignalProducer<String?, CarthageError>.try {
					...
					|> flatMap(.Concat) { refName -> SignalProducer<PinnedVersion, CarthageError> in
						if let refName = refName {
							return self.resolvedGitReference(dependency.project, refName, verbose)
						}

						return self.versionsForDependency(dependency.project, verbose)
							|> collect
	...
	private func graphPermutationsForDependenciesOfNode(node: DependencyNode, basedOnGraph inputGraph: DependencyGraph, verbose: Bool) -> SignalProducer<DependencyGraph, CarthageError> {
		let scheduler = QueueScheduler(name: "org.carthage.CarthageKit.Resolver.graphPermutationsForDependenciesOfNode")

		return cartfileForDependency(node.dependencyVersion, verbose)
			|> startOn(scheduler)
			|> concat(SignalProducer(value: Cartfile(dependencies: [])))
			|> take(1)
			|> observeOn(scheduler)
			|> flatMap(.Merge) { self.nodePermutationsForCartfile($0, verbose: verbose) }
			|> flatMap(.Concat) { dependencyNodes in
				return self.graphPermutationsForEachNode(dependencyNodes, dependencyOf: node, basedOnGraph: inputGraph, verbose: verbose)
					|> promoteErrors(CarthageError.self)
			}
			// Pass through resolution errors only if we never got
			// a valid graph.
	...
	private func graphPermutationsForEachNode(nodes: [DependencyNode], dependencyOf: DependencyNode?, basedOnGraph inputGraph: DependencyGraph, verbose: Bool) -> SignalProducer<Event<DependencyGraph, CarthageError>, NoError> {
		return SignalProducer<(DependencyGraph, [DependencyNode]), CarthageError> { observer, disposable in
			...
			|> flatMap(.Concat) { graph, nodes -> SignalProducer<DependencyGraph, CarthageError> in
				return SignalProducer(values: nodes)
					...
					|> map { node in self.graphPermutationsForDependenciesOfNode(node, basedOnGraph: graph, verbose: verbose) }
					|> collect
