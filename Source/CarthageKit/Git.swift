...
func stringFromGit(input: SignalProducer<TaskEvent<NSData>, CarthageError>) -> SignalProducer<String, CarthageError> {
	return input
		|> map { taskEvent in
...
public func launchGitTask(arguments: [String], repositoryFileURL: NSURL? = nil, standardInput: SignalProducer<NSData, NoError>? = nil, environment: [String: String]? = nil) -> SignalProducer<TaskEvent<NSData>, CarthageError> {
	...
	let gitCallHeaderEvent = TaskEvent<NSData>.StandardOutput(gitCallStr.dataUsingEncoding(NSUTF8StringEncoding)!)
	...
	return SignalProducer<TaskEvent<NSData>, ReactiveTaskError>(value: gitCallHeaderEvent)
		|> concat(launchTask(taskDescription))
		|> mapError { .TaskError($0) }
}

/// Returns a signal that completes when cloning completes successfully.
public func cloneRepository(cloneURL: GitURL, destinationURL: NSURL, bare: Bool = true) -> SignalProducer<TaskEvent<NSData>, CarthageError> {
	...
	return launchGitTask(arguments + [ cloneURL.URLString, destinationURL.path! ])
}

/// Returns a signal that completes when the fetch completes successfully.
public func fetchRepository(repositoryFileURL: NSURL, remoteURL: GitURL? = nil, refspec: String? = nil) -> SignalProducer<TaskEvent<NSData>, CarthageError> {
...
public func listTags(repositoryFileURL: NSURL) -> SignalProducer<TaskEvent<String>, CarthageError> {
	return launchGitTask([ "tag" ], repositoryFileURL: repositoryFileURL)
		|> flatMap(.Concat) { taskEvent -> SignalProducer<TaskEvent<String>, CarthageError> in
			return SignalProducer { observer, disposable in
				switch taskEvent {
				case let .StandardOutput(data):
					sendNext(observer, TaskEvent.StandardOutput(data))
					break
					
				case let .StandardError(data):
					sendNext(observer, TaskEvent.StandardError(data))
					break
					
				case let .Success(box):
					let allTags = NSString(data: box.value, encoding: NSUTF8StringEncoding)!
					allTags.enumerateSubstringsInRange(NSMakeRange(0, allTags.length), options: NSStringEnumerationOptions.ByLines | NSStringEnumerationOptions.Reverse) { line, substringRange, enclosingRange, stop in
						if disposable.disposed {
							stop.memory = true
						}
						
						sendNext(observer, TaskEvent.Success(Box<String>(line as String)))
					}
					break
				}
...
/// will be created.
public func checkoutRepositoryToDirectory(repositoryFileURL: NSURL, workingDirectoryURL: NSURL, revision: String = "HEAD", shouldCloneSubmodule: Submodule -> Bool = { _ in true }) -> SignalProducer<TaskEvent<NSData>, CarthageError> {
	return SignalProducer.try {
		...
		|> flatMap(.Concat) { environment in launchGitTask([ "checkout", "--force", revision ], repositoryFileURL: repositoryFileURL, environment: environment) }
...
public func cloneSubmodulesForRepository(repositoryFileURL: NSURL, workingDirectoryURL: NSURL, revision: String = "HEAD", shouldCloneSubmodule: Submodule -> Bool = { _ in true }) -> SignalProducer<TaskEvent<NSData>, CarthageError> {
	return submodulesInRepository(repositoryFileURL, revision: revision)
		|> flatMap(.Concat) { submodule -> SignalProducer<TaskEvent<NSData>, CarthageError> in
...
public func cloneSubmoduleInWorkingDirectory(submodule: Submodule, workingDirectoryURL: NSURL) -> SignalProducer<TaskEvent<NSData>, CarthageError> {
	...
	let purgeGitDirectories = NSFileManager.defaultManager().carthage_enumeratorAtURL(submoduleDirectoryURL, includingPropertiesForKeys: [ NSURLIsDirectoryKey, NSURLNameKey ], options: nil, catchErrors: true)
		|> flatMap(.Merge) { enumerator, URL -> SignalProducer<TaskEvent<NSData>, CarthageError> in
			var name: AnyObject?
			var error: NSError?
...
/// directory.
private func checkoutSubmodule(submodule: Submodule, submoduleWorkingDirectoryURL: NSURL) -> SignalProducer<TaskEvent<NSData>, CarthageError> {
	return launchGitTask([ "checkout", submodule.SHA ], repositoryFileURL: submoduleWorkingDirectoryURL)
		|> then(launchGitTask([ "submodule", "update", "--init", "--recursive" ], repositoryFileURL: submoduleWorkingDirectoryURL))
}
...
public func addSubmoduleToRepository(repositoryFileURL: NSURL, submodule: Submodule, fetchURL: GitURL) -> SignalProducer<TaskEvent<NSData>, CarthageError> {
		...
		|> flatMap(.Merge) { submoduleExists in
			if (submoduleExists) {
				...
				return fetchRepository(submoduleDirectoryURL, remoteURL: fetchURL, refspec: "+refs/heads/*:refs/remotes/origin/*")
					...
					|> then(launchGitTask([ "submodule", "sync" ], repositoryFileURL: repositoryFileURL))
					|> then(checkoutSubmodule(submodule, submoduleDirectoryURL))
					|> then(launchGitTask([ "add", "--force", submodule.path ], repositoryFileURL: repositoryFileURL))
			} else {
				let addSubmodule = launchGitTask([ "submodule", "add", "--force", "--name", submodule.name, "--", submodule.URL.URLString, submodule.path ], repositoryFileURL: repositoryFileURL)
					// A .failure to add usually means the folder was already added
					// to the index. That's okay.
					|> catch { _ in SignalProducer<TaskEvent<NSData>, CarthageError>.empty }
				...
				return cloneRepository(fetchURL, submoduleDirectoryURL, bare: false)
					...
					|> then(checkoutSubmodule(submodule, submoduleDirectoryURL))
					|> then(addSubmodule)
					|> then(launchGitTask([ "submodule", "init", "--", submodule.path ], repositoryFileURL: repositoryFileURL))
			}
		}
}
