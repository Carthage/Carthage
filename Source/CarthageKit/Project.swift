...
public final class Project {
	...
	public func buildCheckedOutDependenciesWithConfiguration(configuration: String, forPlatform platform: Platform?, sendWarning: (String -> Void)? = nil) -> SignalProducer<BuildSchemeProducer, CarthageError> {
		return loadResolvedCartfile()
			...
			|> flatMap(.Concat) { dependency -> SignalProducer<BuildSchemeProducer, CarthageError> in
				...
				return buildDependencyProject(dependency.project, self.directoryURL, withConfiguration: configuration, platform: platform, sendWarning: sendWarning)
			}
	}
}
