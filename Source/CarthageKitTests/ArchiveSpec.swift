	...
	override func spec() {
		...
		describe("zipping") {
			...
			it("should zip relative paths into an archive") {
				...
				expect { try "foobar".write(toFile: innerFilePath, atomically: true, encoding: NSUTF8StringEncoding) }.notTo(throwError())

				let outerFilePath = "outer"
				expect { try "foobar".write(toFile: outerFilePath, atomically: true, encoding: NSUTF8StringEncoding) }.notTo(throwError())
			...
			it("should preserve symlinks") {
				...
				expect { try "foobar".write(toFile: destinationPath, atomically: true, encoding: NSUTF8StringEncoding) }.notTo(throwError())
