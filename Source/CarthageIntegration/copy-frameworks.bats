...
setup() {
    ...
    git clone -b 1.0.0 https://github.com/ikesyo/CarthageCopyFrameworksFixture.git
...
@test "carthage copy-frameworks should work for all an app target, a unit test target, a UI test target and produce a valid xcarchive file" {
    ...
    run xcodebuild clean archive -scheme CarthageCopyFrameworksFixture -archivePath CarthageCopyFrameworksFixture.xcarchive
    [ "$status" -eq 0 ]
